    ...
    <transition name="fade-right">
        <div class="scheduleDragContainer">
            <header class="scheduleHeader d-flex">
                 <div class="me-3 ms-3" :class="{'activeSchedule':i.ID==currentSchedule.ID}" v-for="(i) in scheduleList" :key="i" @click="selectSchedule(i)">{{i.scheduleTitle}}</div>
            </header>
            <main class="scheduleItem-body">
                    <ScheduleItem v-for="(i) in scheduleList" :key="i"
                        :scheduleData="i"
